...
// Shared webhook handler logic — single source of truth for both v1 and v2 routes.
// Handlers internally check app.webhookVersion to decide which notification format to use.
...
export interface V2WebhookEvent {
  type: string
  data: {
    transaction?: {
      id: string
...
// ── App shape for webhook delivery ──────────────────────────────
...
// ── Shared webhook delivery mechanism ───────────────────────────
...
export async function notifyDeveloperV1(
  app: WebhookApp,
  eventType: string,
  payload: Record<string, unknown>
) {
  await deliverWebhook(app, { type: eventType, data: payload }, eventType)
}

// ── V2 Notify Developer ─────────────────────────────────────────

export async function notifyDeveloperV2(
  app: WebhookApp,
  event: V2WebhookEvent
) {
  const v2Payload = {
    id: `evt_${randomUUID()}`,
    version: "2026-03-08",
  ...
  await deliverWebhook(app, v2Payload, event.type)
}
...
export async function handleCheckoutSessionCompleted(
  ...
  let entCurrentPeriodEnd: string | null = null
  let entCancelAtPeriodEnd = false
  ...
  if (product.productType === 'SUBSCRIPTION' && session.subscription) {
    ...
    entCurrentPeriodEnd = firstItem
      ? new Date(firstItem.current_period_end * 1000).toISOString()
      : null
    entCancelAtPeriodEnd = subscription.cancel_at_period_end
  } else {
  ...
  // Notify developer webhook — v1 or v2 based on webhookVersion
  const withdrawalWaiverField = session.custom_fields?.find(
    (f) => f.key === 'withdrawal_waiver'
  )

  if (product.app.webhookVersion === "v2") {
    await notifyDeveloperV2(product.app, {
      ...
      data: {
        ...
        entitlement: {
          status: "active",
          currentPeriodEnd: entCurrentPeriodEnd,
          cancelAtPeriodEnd: entCancelAtPeriodEnd,
        },
        withdrawalWaiver: {
    ...
    await notifyDeveloperV1(product.app, 'checkout.session.completed', {
      userId: externalUserId,
      productId: product.id,
      europay: {
...
// No developer notification in v1 or v2 — internal tracking only

export async function handleSubscriptionUpdated(subscription: Stripe.Subscription): Promise<string | null> {
...
export async function handleSubscriptionDeleted(subscription: Stripe.Subscription): Promise<string | null> {
  const entitlement = await prisma.entitlement.findUnique({
    where: { stripeSubscriptionId: subscription.id },
  ...
  // Notify developer webhook — v1 or v2
  if (entitlement.customer.app.webhookVersion === "v2") {
    await notifyDeveloperV2(entitlement.customer.app, {
    ...
    await notifyDeveloperV1(entitlement.customer.app, 'customer.subscription.deleted', {
      userId: entitlement.customer.externalUserId,
      productId: entitlement.productId,
      europay: {
...
export async function handleInvoicePaymentSucceeded(
  ...
  // Notify developer webhook — v2 only (v1 has no notification for renewals)
  if (entitlement.customer.app.webhookVersion === "v2") {
    await notifyDeveloperV2(entitlement.customer.app, {
      ...
      data: {
        transaction: {
          id: renewalTx.id,
          amount: renewalTx.amountTotal,
          currency: renewalTx.currency,
          status: "succeeded",
          createdAt: renewalTx.createdAt.toISOString(),
        },
        product: {
...
export async function handleInvoicePaymentFailed(invoice: Stripe.Invoice): Promise<string | null> {
  const subDetails = invoice.parent?.subscription_details
  ...
  if (entitlement.customer.app.webhookVersion === "v2") {
    ...
    await notifyDeveloperV1(entitlement.customer.app, 'invoice.payment_failed', {
      userId: entitlement.customer.externalUserId,
      productId: entitlement.productId,
...
export async function handleChargeRefunded(
  ...
  // Notify developer webhook — v1 or v2
  if (transaction.customer.app.webhookVersion === "v2") {
    await notifyDeveloperV2(transaction.customer.app, {
    ...
    await notifyDeveloperV1(transaction.customer.app, 'charge.refunded', {
      userId: transaction.customer.externalUserId,
...
export async function handleDisputeCreated(
  ...
  if (transaction.customer.app.webhookVersion === "v2") {
    ...
    await notifyDeveloperV1(transaction.customer.app, 'charge.dispute.created', {
      userId: transaction.customer.externalUserId,
      productId: transaction.productId,
      transactionId: transaction.id,
...
// ── Helper: report transaction to Apple ─────────────────────────

export async function reportToApple(appId: string, transactionId: string) {
  try {
